...
export default function EditViolation({ violation, students, violationTypes, statuses }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Edit',
            href: `/violations/${violation.id}/edit`,
        },
    ];

    const [selectedStudent, setSelectedStudent] = useState<Student | null>(violation.student || null);
    const [searchTerm, setSearchTerm] = useState(violation.student_name);
    const [showStudentDropdown, setShowStudentDropdown] = useState(false);

    const { data, setData, put, processing, errors } = useForm<ViolationFormData>({
        student_id: violation.student_id.toString(),
        student_name: violation.student_name,
        student_class: violation.student_class,
        violation_date: violation.violation_date,
        violation_type: violation.violation_type,
        description: violation.description,
        disciplinary_action: violation.disciplinary_action,
        reporting_staff: violation.reporting_staff,
    ...
    return (
        <AppShell breadcrumbs={breadcrumbs}>
            <Head title={`Edit Pelanggaran #${violation.id}`} />
            
            <div className="max-w-4xl mx-auto space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">✏️ Edit Pelanggaran</h1>
                    <p className="text-gray-600">Perbarui informasi pelanggaran siswa</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow border">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Action, Reporter, and Status */}
                        <div>
                            <h2 className="text-lg font-semibold text-gray-900 mb-4">⚖️ Tindakan, Pelapor & Status</h2>
                        ...
                        <div className="border-t pt-6">
                            <div className="flex justify-end space-x-3">
                                ...
                                >
                                    {processing ? '⏳ Menyimpan...' : '💾 Update Pelanggaran'}
                                </Button>
                            </div>
                        </div>
                    </form>
